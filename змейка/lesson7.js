...
function respawn() {
    ...
    snake_head.setAttribute('class', snake_head.getAttribute('class') + ' snake-unit new');
    // Тело змейки
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && !haveBomb(new_unit)) {
        ...
        new_unit.setAttribute('class', new_unit.getAttribute('class') + ' snake-unit new');
        snake.push(new_unit);
        ...
        document.querySelector('.new').classList.replace('new', 'booh')
        new_unit.setAttribute('class', new_unit.getAttribute('class') + ' snake-unit booh');
        snake.push(new_unit);
        setTimeout(finishTheGame, 500)
...
function finishTheGame() {
    ...
    alert(' Ваш результат: ' + score.toString());
